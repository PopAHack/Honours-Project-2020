...
public class DistanceDataCollector {
    ...
    public DistanceDataCollector(RouteNetwork network, Disease disease) {
        try {
            ...
            // This is where the flight API would be called, and flights calculated and filtered.
            // For now, we will randomly generate realistic data from normal distributions.`
            ...
            for (int i = 0; i < numCities - 1; i++) {
                CityNode cityNode = new CityNode(randNameGenerator(rand, false), (int) (rand.nextGaussian() * 300000 + 1000000), disease);
            }
            CityNode cityNode = new CityNode(randNameGenerator(rand, true), (int) (rand.nextGaussian() * 300000 + 1000000), disease);
            ...
            for (int i = 0; i < numFlights; i++) {
                ...
                while (indexT < 0 || indexT > numCities - 1 || indexS < 0 || indexS > numCities - 1) {
                    ...
                    indexS = (int) (rand.nextGaussian() * 150 + 600);
                }
                CityNode sourceCity = CityNode.get(indexS);
                CityNode targetCity = CityNode.get(indexT);

                Path path = new Path(sourceCity, targetCity, rand.nextGaussian() * 100 + 300, disease);
                if (sourceCity.getName().equals(CityNode.getCenterTarget().getName()) && path.getTransportDistance() > 0)
